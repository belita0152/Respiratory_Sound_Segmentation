...
from data.hylee.utils.loss import CEDiceLoss
...
def get_args():
    ...
    parser.add_argument("--model_name", type=str, default="unet2d_mel")
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.model: nn.Module = UNet2D(**self.model_cfg).to(self.device)
    ...
    def train_one_epoch(self, epoch: int):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader, start=1):
            ...
            x = self._make_input(data)  # mel: [B, C, n_mels, n_frames]
            y = target.long().to(self.device, non_blocking=True)

            with torch.cuda.amp.autocast(enabled=self.device.type == "cuda"):
                logits = self.model(x)  # [B, num_classes, n_frames]
    ...
    def eval_one_epoch(self, epoch: int) -> Dict:
        ...
        iou_macro = result["iou_macro"]
        dice_macro = result["dice_macro"]
        per_class_str = " ".join(
            [
    ...
    def _build_dataloaders(self, use_train_sampler=True) -> Tuple[Tuple[DataLoader, DataLoader], Tuple[int, int]]:
        input_transform = WindowTransform(
            ...
            normalize=not self.args.no_normalize,
    ...
    def _count_dataset_labels(self, dataset, num_classes: int, ignore_index: int = -1):
    ...
    def _count_loader_labels(self, loader, num_classes: int, ignore_index: int = -1, max_batches: int = 20):
        counts = torch.zeros(num_classes, dtype=torch.long)
